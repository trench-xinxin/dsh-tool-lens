...
const LENS_OUTPUT_SCHEMA = {
	...
	properties: {
		...
		pathfinding: { type: "object" },
		deadCode: { type: "object" },
		architectureViolations: { type: "array" }
	}
};
/**
...
function apply(ctx, config = {}) {
	...
	ctx.tools.register(defineTool({
		name: "lens",
		description: "Inspect symbol call hierarchies, file dependencies, circular dependencies, architecture metrics, shortest call paths, dead code, and refactoring impact graphs across TypeScript, Vue, Svelte, Python, Go, Rust, and Java codebases.",
		parameters: {
			action: {
				type: "string",
				required: true,
				...
				description: "The type of graph query: dependencies (file imports), call_graph (function calls), impact (blast radius), circular (cycle audit), metrics (coupling health), path (shortest invocation trace), unused (dead code audit), or lint (layer boundary rules)."
			},
			target: {
				type: "string",
...
//#endregion
export { CodeAnalyzer, Config, ConfigParser, DriverRegistry, GoLanguageDriver, GraphStore, IncrementalCacheStore, JavaLanguageDriver, LENS_PROMPT_TEXT, LensWatcher, PythonLanguageDriver, RustLanguageDriver, SFCLanguageDriver, SUPPORTED_EXTENSIONS, TSLanguageDriver, TSParser, analyzeCircularDependencies, analyzeImpact, analyzeProjectMetrics, apply, buildCircularResult, buildLintResult, buildMetricsResult, buildPathfindingResult, buildUnusedResult, extractSFCBlocks, formatGraphMarkdown, generateMermaidDiagram, inject, kebabToPascal, name, parseGoSource, parseJavaSource, parsePythonSource, parseRustSource, presentLensCall, presentLensResult, resolveModulePath };
