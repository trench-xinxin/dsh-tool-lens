	...
	*/
	...
	findNodes(query) {
		...
		for (const node of this.nodes.values()) {
			...
			if ((node.kind === "file" || node.kind === "component") && (nodePath.endsWith(`/${normalized}`) || nodePath === normalized || nodePath.endsWith(`/${cleanQuery.toLowerCase()}`))) {
			...
			if (nodeName.includes(normalized) || nodePath.includes(normalized)) fuzzyInclusionMatches.push(node);
		}
...
function resolveJavaModulePath(currentRelPath, specifier, rootDir, knownFiles) {
	const relPath = specifier.replace(/\.\*$/, "").replace(/\./g, "/");
	const exactMaven = probeFileVariants(join(rootDir, "src/main/java", relPath), [".java"]);
	if (exactMaven) return normalize(relative(rootDir, exactMaven));
	const exactRoot = probeFileVariants(join(rootDir, relPath), [".java"]);
	if (exactRoot) return normalize(relative(rootDir, exactRoot));
	if (knownFiles) {
		const candidateJava = `${relPath}.java`;
		for (const known of knownFiles) if (known.endsWith(candidateJava) || known.endsWith(`/${candidateJava}`)) return known;
	}
	return `${relPath}.java`;
}
...
//#endregion
//#region src/parsers/java-parser.ts
/**
* Parses Java source code into symbols, imports, heritages, and calls.
*/
function parseJavaSource(content, _relPath) {
	const lines = content.split(/\r?\n/);
	const symbols = [];
	const imports = [];
	const heritages = [];
	...
	for (let i = 0; i < lines.length; i++) {
		...
		const classMatch = trimmed.match(/^(?:public|protected|private|abstract|final|static|\s)*\bclass\s+([a-zA-Z0-9_]+)(?:<[^>]*>)?(?:\s+extends\s+([a-zA-Z0-9_]+(?:<[^>]*>)?))?(?:\s+implements\s+([a-zA-Z0-9_,\s<>]+))?/);
		if (classMatch) {
			...
			if (extendsClass) {
				...
				if (cleanExtends && cleanExtends !== "Object") heritages.push({
			...
			if (implementsIfaces) {
				const ifaceList = implementsIfaces.replace(/<[^>]*>/g, "").split(",").map((item) => item.trim()).filter(Boolean);
				for (const iface of ifaceList) heritages.push({
					sourceName: className,
					targetName: iface,
					relation: "implements"
				});
		...
		const ifaceMatch = trimmed.match(/^(?:public|protected|private|abstract|static|\s)*\binterface\s+([a-zA-Z0-9_]+)(?:<[^>]*>)?(?:\s+extends\s+([a-zA-Z0-9_,\s<>]+))?/);
		if (ifaceMatch) {
			const ifaceName = ifaceMatch[1];
			...
			if (extendsList) {
				const superIfaces = extendsList.replace(/<[^>]*>/g, "").split(",").map((item) => item.trim()).filter(Boolean);
				for (const sup of superIfaces) heritages.push({
		...
		const methodMatch = trimmed.match(/^(?:@\w+(?:\([^)]*\))?\s+)*(?:public|protected|private|static|final|abstract|synchronized|native|\s)*\s*([a-zA-Z0-9_<>[\]]+)\s+([a-zA-Z0-9_]+)\s*\([^)]*\)(?:\s+throws\s+[a-zA-Z0-9_,\s]+)?(?:\s*\{|\s*;)/);
		...
		if (currentMethod) extractJavaCalls(trimmed, currentClass?.name, (calleeName, calleeObject) => {
			calls.push({
...
function extractJavaCalls(line, currentClassName, onCall) {
	const callRegex = /\b([a-zA-Z0-9_]+(?:\.[a-zA-Z0-9_]+)*)\s*\(/g;
	let match;
	while ((match = callRegex.exec(line)) !== null) {
		const rawExpr = match[1];
		...
		if (rawExpr.includes(".")) {
			...
			if (obj === "this" && currentClassName) onCall(calleeName, currentClassName);
			...
			if (!targetNode && fileBindings) {
				if (!call.calleeObject) {
					...
					if (classOrNsBinding) {
						...
						targetNode = targetFileSymbols?.get(`${classOrNsBinding.importedName}.${call.calleeName}`) || targetFileSymbols?.get(`${call.calleeObject}.${call.calleeName}`) || targetFileSymbols?.get(call.calleeName);
					}
				}
			}
			if (!targetNode) {
...
//#endregion
//#region src/analytics/api-contracts.ts
/**
...
function normalizeApiPath(path) {
	return path.trim().replace(/\{[^}]+\}/g, "*").replace(/:[a-zA-Z0-9_]+/g, "*").replace(/\/+/g, "/").replace(/\/$/, "");
}
/**
* Scans files in the indexed workspace to match frontend client calls to backend server handlers.
*/
function buildApiContractsResult(graph, workspaceRootOrFiles, explicitFileSources) {
	const clientCalls = [];
	const serverEndpoints = [];
	let workspaceRoot;
	let fileSources;
	if (workspaceRootOrFiles instanceof Map) fileSources = workspaceRootOrFiles;
	else if (typeof workspaceRootOrFiles === "string") {
		workspaceRoot = workspaceRootOrFiles;
		fileSources = explicitFileSources;
	}
	const sourcesToScan = /* @__PURE__ */ new Map();
	if (fileSources && fileSources.size > 0) for (const [p, content] of fileSources.entries()) sourcesToScan.set(p, content);
	else if (workspaceRoot) {
		for (const node of graph.getAllNodes()) if (node.kind === "file" || node.kind === "component") try {
			const absPath = isAbsolute(node.filePath) ? node.filePath : resolve(workspaceRoot, node.filePath);
			const content = readFileSync(absPath, "utf8");
	...
	for (const [filePath, content] of sourcesToScan.entries()) {
		...
		for (let i = 0; i < lines.length; i++) {
			const line = lines[i];
			const lineNum = i + 1;
			const clientMatch = line.match(/\b(?:axios\.(get|post|put|delete|patch)|request\.(get|post|put|delete|patch)|http\.(get|post|put|delete|patch)|fetch)\s*\(\s*['"`]([^'"`]+)['"`]/i);
			if (clientMatch) {
				const method = (clientMatch[1] || clientMatch[2] || clientMatch[3] || "GET").toUpperCase();
				const url = clientMatch[4];
				if (url.startsWith("/") || url.startsWith("http") || url.includes("/api/")) clientCalls.push({
					filePath,
					url,
					method,
					line: lineNum
			...
			const springMatch = line.match(/@(?:(Get|Post|Put|Delete|Patch)Mapping|RequestMapping)\s*\(\s*(?:(?:value|path)\s*=\s*)?["']([^"']+)["']/i);
			if (springMatch) {
			...
			const goMatch = line.match(/\b(?:r|router|e|app|group|engine)\.(GET|POST|PUT|DELETE|PATCH)\s*\(\s*["']([^"']+)["']/i);
			if (goMatch) {
	...
	const matchedClientIndices = /* @__PURE__ */ new Set();
	const matchedServerIndices = /* @__PURE__ */ new Set();
	for (let cIdx = 0; cIdx < clientCalls.length; cIdx++) {
		...
		for (let sIdx = 0; sIdx < serverEndpoints.length; sIdx++) {
			...
			if (clientNorm === normalizeApiPath(server.url) && (server.method === "ALL" || client.method === server.method)) {
	...
	const unmatchedClientCalls = clientCalls.filter((_, idx) => !matchedClientIndices.has(idx)).map((c) => ({
	...
	const unmatchedServerEndpoints = serverEndpoints.filter((_, idx) => !matchedServerIndices.has(idx)).map((s) => ({
	...
	return {
		...
		nodes: [...matchedContracts.map((m) => m.clientCallNode), ...matchedContracts.map((m) => m.serverHandlerNode)],
...
//#endregion
//#region src/analytics/architecture.ts
...
*/
function buildLintResult(graph, rawRules) {
	let rules = [];
	...
	else if (Array.isArray(rawRules)) rules = rawRules;
	...
	for (const edge of allEdges) {
		...
		if (!fromNode || !toNode || fromNode.filePath === toNode.filePath) continue;
		for (const item of compiledRules) {
			...
			if (fromMatches && toMatches) violations.push({
				fromNode,
				toNode,
	...
	let summary = "";
	if (violations.length === 0) summary = `✅ Architecture compliant: Evaluated ${rules.length} rule(s) across ${allEdges.length} edge(s) with 0 violations.`;
	else summary = `🚨 Found ${violations.length} architecture boundary violation(s) across ${rules.length} evaluated rule(s).`;
	return {
		target: "architecture",
		action: "lint",
		rootNodes: [],
		nodes: violations.map((v) => v.fromNode),
		edges: violations.map((v) => ({
			from: v.fromNode.id,
			to: v.toNode.id,
			relation: v.relation
		})),
...
//#endregion
//#region src/analytics/deadcode.ts
...
*/
function buildUnusedResult(graph, scope) {
	...
	for (const node of allNodes) {
		...
		if (node.kind === "file" || node.kind === "component") {
			if (!isEntryPoint(node.filePath)) {
				if (graph.getInboundEdges(node.id).filter((e) => e.relation === "imports").length === 0) orphanFiles.push(node);
			}
			continue;
		}
		if (node.kind === "function" || node.kind === "class" || node.kind === "interface") {
			if (graph.getInboundEdges(node.id).filter((e) => e.relation === "calls" || e.relation === "implements" || e.relation === "extends").length === 0 && !isEntryPoint(node.filePath)) unusedSymbols.push(node);
		}
	}
	const deadCodeData = {
		orphanFiles,
	...
	let summary = "";
	if (orphanFiles.length === 0 && unusedSymbols.length === 0) summary = "✅ Clean architecture: No dead code or orphan files detected.";
	else summary = `⚠️ Detected ${orphanFiles.length} orphan file(s) and ${unusedSymbols.length} unreachable symbol(s).`;
	return {
...
//#endregion
//#region src/analytics/git-diff.ts
/**
...
function isTestFile(filePath) {
	return filePath.includes(".spec.") || filePath.includes(".test.") || filePath.includes("__tests__") || filePath.endsWith("_test.go") || filePath.endsWith("Test.java") || filePath.endsWith("Tests.java");
}
/**
* Extracts changed files and symbols from git diff and calculates upstream impact.
*/
function analyzeGitDiffImpact(graph, workspaceRoot, commit, rawChangedFiles) {
	let changedFilePaths = [];
	if (rawChangedFiles && rawChangedFiles.length > 0) changedFilePaths = rawChangedFiles.map((f) => normalize(f));
	else try {
		if (commit) changedFilePaths = execSync(`git diff --name-only ${commit}`, {
			cwd: workspaceRoot,
			encoding: "utf8",
			stdio: [
				"pipe",
		...
		else changedFilePaths = execSync("git status --porcelain", {
			cwd: workspaceRoot,
			encoding: "utf8",
			stdio: [
				"pipe",
	...
	const changedSymbolNodes = [];
	const affectedUpstreamFilesSet = /* @__PURE__ */ new Set();
	const breakingCallers = [];
	const affectedTestFilesSet = /* @__PURE__ */ new Set();
	for (const filePath of uniqueChangedFiles) {
		const fileNode = graph.getNode(filePath);
		if (fileNode) changedSymbolNodes.push(fileNode);
		const childSymbols = graph.findNodes(filePath);
		for (const sym of childSymbols) if (sym.filePath === filePath && sym.kind !== "file" && sym.kind !== "component") changedSymbolNodes.push(sym);
	}
	const changedNodeIds = changedSymbolNodes.map((n) => n.id);
	if (changedNodeIds.length > 0) {
		const traversal = graph.traverse(changedNodeIds, "inbound", 3);
		for (const node of traversal.nodes) if (!uniqueChangedFiles.includes(node.filePath)) {
			affectedUpstreamFilesSet.add(node.filePath);
			if (isTestFile(node.filePath)) affectedTestFilesSet.add(node.filePath);
		}
		for (const edge of traversal.edges) if (edge.relation === "calls" && !uniqueChangedFiles.some((f) => edge.from.startsWith(f))) {
			const callerNode = graph.getNode(edge.from);
			if (callerNode && !breakingCallers.some((b) => b.id === callerNode.id)) breakingCallers.push(callerNode);
		}
	}
	...
	let summary = "";
	if (uniqueChangedFiles.length === 0) summary = "Working tree clean: No changed files detected in git diff.";
	else summary = `Git diff contains ${uniqueChangedFiles.length} modified file(s), impacting ${affectedUpstreamFiles.length} upstream file(s) across ${breakingCallers.length} breaking caller(s). Recommended ${affectedTestFiles.length} regression test file(s).`;
	return {
...
//#endregion
//#region src/analytics/pathfinding.ts
/**
* Explores and computes the shortest invocation path from source node to target node.
*/
function buildPathfindingResult(graph, fromQuery, toQuery, maxHops = 10) {
...
//#endregion
//#region src/analytics/slicing.ts
/**
* Finds domain seed nodes by exact match or fuzzy path/name inclusion.
*/
function findDomainSeedNodes(graph, domainQuery) {
	...
	for (const node of graph.getAllNodes()) {
		...
		if (p.includes(`/${q}/`) || p.includes(`/${q}.`) || p.startsWith(`${q}/`) || p.includes(q) || n.includes(q) || id.includes(q)) matches.push(node);
	}
	return matches;
}
/**
...
*/
function buildSliceResult(graph, domainQuery, maxHops = 2) {
	...
	const internalEdges = [];
	const boundaryOutgoingEdges = [];
	for (const node of slicedNodes) {
		const outEdges = graph.getOutboundEdges(node.id);
		for (const edge of outEdges) if (slicedNodeIds.has(edge.to)) internalEdges.push(edge);
		else boundaryOutgoingEdges.push(edge);
	}
	const totalEdges = internalEdges.length + boundaryOutgoingEdges.length;
	const cohesionScore = totalEdges > 0 ? internalEdges.length / totalEdges : 1;
	const sliceResult = {
...
function formatGraphMarkdown(result) {
	...
	if (result.action === "path") return formatPathMarkdown(result);
	if (result.action === "unused" && result.deadCode) return formatUnusedMarkdown(result);
	if (result.action === "lint") return formatLintMarkdown(result);
	if (result.action === "diff_impact" && result.diffImpact) return formatDiffImpactMarkdown(result);
	if (result.action === "slice" && result.sliceResult) return formatSliceMarkdown(result);
	if (result.action === "api_contracts" && result.apiContracts) return formatApiContractsMarkdown(result);
	const lines = [
...
function formatDiffImpactMarkdown(result) {
	...
	if (d.breakingCallers.length > 0) {
		...
		for (const c of d.breakingCallers) lines.push(`- 🔴 **[${c.kind}]** \`${c.name}\` (\`${c.filePath}${c.line ? `:${c.line}` : ""}\`)`);
		lines.push("");
	}
	if (d.affectedTestFiles.length > 0) {
		lines.push(`**🎯 Recommended Regression Test Suite (${d.affectedTestFiles.length}):**`);
		for (const t of d.affectedTestFiles) lines.push(`- 🧪 \`${t}\``);
		lines.push("");
	}
	return lines.join("\n");
}
function formatSliceMarkdown(result) {
	...
	for (const n of s.slicedNodes.slice(0, 25)) lines.push(`- [${n.kind}] \`${n.name}\` (\`${n.filePath}\`)`);
	if (s.slicedNodes.length > 25) lines.push(`- *... and ${s.slicedNodes.length - 25} more domain symbols*`);
	lines.push("");
	const mermaid = generateMermaidDiagram(s.slicedNodes, s.internalEdges, 25);
	if (mermaid) lines.push("#### Domain Slice Topology", mermaid);
	return lines.join("\n");
}
function formatApiContractsMarkdown(result) {
	...
	if (a.matchedContracts.length > 0) {
		...
		lines.push("| :--- | :--- | :--- | :--- |");
		for (const c of a.matchedContracts) lines.push(`| **${c.httpMethod}** | \`${c.urlPattern}\` | \`${c.clientCallNode.filePath}:${c.clientCallNode.line}\` | \`${c.serverHandlerNode.filePath}:${c.serverHandlerNode.line}\` |`);
		lines.push("");
	}
	if (a.unmatchedClientCalls.length > 0) {
		lines.push(`**Unmatched Frontend API Calls (${a.unmatchedClientCalls.length}):**`);
		for (const c of a.unmatchedClientCalls.slice(0, 10)) lines.push(`- ⚠️ \`${c.name}\` at \`${c.filePath}:${c.line}\``);
		lines.push("");
	}
	const mermaid = generateMermaidDiagram(result.nodes, result.edges, 25);
	if (mermaid) lines.push("#### Full-Stack Contract Topology", mermaid);
	return lines.join("\n");
}
function formatPathMarkdown(result) {
	...
	if (mermaid) lines.push("#### Path Topology", mermaid);
	return lines.join("\n");
}
function formatUnusedMarkdown(result) {
	...
	if (d.orphanFiles.length > 0) {
		...
		for (const f of d.orphanFiles) lines.push(`- 📄 \`${f.filePath}\` (no external imports)`);
		lines.push("");
	}
	if (d.unusedSymbols.length > 0) {
		...
		lines.push("| :--- | :--- | :--- | :--- |");
		for (const sym of d.unusedSymbols.slice(0, 30)) lines.push(`| ${sym.kind} | \`${sym.name}\` | \`${sym.filePath}\` | ${sym.line ?? "-"} |`);
		if (d.unusedSymbols.length > 30) lines.push(`| ... | *${d.unusedSymbols.length - 30} more unreferenced symbols* | | |`);
		lines.push("");
	}
	return lines.join("\n");
}
function formatLintMarkdown(result) {
...
function formatCircularMarkdown(result) {
	...
	const mermaid = generateMermaidDiagram(result.nodes, result.edges, 20);
	if (mermaid) lines.push("#### Circular Topology", mermaid);
	return lines.join("\n");
}
function formatMetricsMarkdown(result) {
	...
	if (m.topHubs.length > 0) {
		...
		m.topHubs.slice(0, 10).forEach((hub, idx) => {
			lines.push(`| #${idx + 1} | ${hub.kind} | \`${hub.name}\` | **${hub.degree}** | ${hub.inboundDegree} | ${hub.outboundDegree} |`);
		});
		lines.push("");
	}
	if (m.modules.length > 0) {
		...
		m.modules.slice(0, 15).forEach((mod) => {
			const assess = mod.instability === 0 ? "🛡 Very Stable" : mod.instability >= .8 ? "⚠️ Fragile" : "⚖ Balanced";
			lines.push(`| \`${mod.filePath}\` | ${mod.afferentCoupling} | ${mod.efferentCoupling} | ${(mod.instability * 100).toFixed(0)}% | ${assess} |`);
		});
		lines.push("");
	}
	return lines.join("\n");
...
function formatImpactMarkdown(result) {
	...
	if (tiers.directBreaking.length > 0) {
		...
		for (const n of tiers.directBreaking) lines.push(`- **[${n.kind}]** \`${n.name}\` (${n.filePath}${n.line ? `:${n.line}` : ""})`);
		lines.push("");
	}
	if (tiers.internalCascading.length > 0) {
		lines.push(`#### 🟡 Tier 1: Internal Cascading (${tiers.internalCascading.length})`);
		for (const n of tiers.internalCascading) lines.push(`- **[${n.kind}]** \`${n.name}\` (${n.filePath}${n.line ? `:${n.line}` : ""})`);
		lines.push("");
	}
	if (tiers.transitiveImporters.length > 0) {
		lines.push(`#### 🔵 Tier 2: Transitive Importers (${tiers.transitiveImporters.length})`);
		for (const n of tiers.transitiveImporters) lines.push(`- **[${n.kind}]** \`${n.name}\` (${n.filePath})`);
		lines.push("");
	}
	const mermaid = generateMermaidDiagram(result.nodes, result.edges, 25);
	if (mermaid) lines.push("#### Impact Topology Flowchart", mermaid);
	return lines.join("\n");
}
//#endregion
...
- **ENVIRONMENT IS JAVASCRIPT / TYPESCRIPT, NOT PYTHON!**
- **NEVER** write Python code (like \`import os\`, \`print(os.getcwd())\`, \`import sys\`).
- **NEVER** write Node.js \`import\` or \`export\` or \`require('fs')\` statements inside \`run_code\`.
- **ALWAYS** return the tool execution result directly in JavaScript:
  \`return await tools.lens({ action: 'impact', target: 'YourSymbol' });\`
`.trim();
...
function resolveWorkspaceRoot(ctx, exec) {
	if (exec?.cwd && typeof exec.cwd === "string" && exec.cwd.trim().length > 0) return exec.cwd.trim();
	const fsService = ctx.get?.("fs");
	if (fsService?.root?.displayPath && typeof fsService.root.displayPath === "string") return fsService.root.displayPath;
	const sessionService = ctx.get?.("session");
	if (sessionService?.cwd && typeof sessionService.cwd === "string") return sessionService.cwd;
	return process.cwd();
}
/**
...
function apply(ctx, config = {}) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			...
			const store = await analyzer.indexDirectory(scanDir, exec?.signal, { forceReindex: !useCache });
			...
			if (lensArgs.action === "unused") return buildUnusedResult(store, lensArgs.scope);
			if (lensArgs.action === "lint") return buildLintResult(store, lensArgs.rules);
			if (lensArgs.action === "diff_impact") return analyzeGitDiffImpact(store, workspaceRoot, lensArgs.commit);
			if (lensArgs.action === "slice") {
				...
				return buildSliceResult(store, targetQuery, Math.min(lensArgs.depth ?? 2, 4));
			}
			if (lensArgs.action === "api_contracts") return buildApiContractsResult(store, workspaceRoot);
			if (lensArgs.action === "path") {
				if (!targetQuery || !lensArgs.to) return {
					target: targetQuery,
					action: "path",
					rootNodes: [],
					nodes: [],
...
//#endregion
export { CodeAnalyzer, Config, ConfigParser, DriverRegistry, GoLanguageDriver, GraphStore, IncrementalCacheStore, JavaLanguageDriver, LENS_PROMPT_TEXT, LensWatcher, PythonLanguageDriver, RustLanguageDriver, SFCLanguageDriver, SUPPORTED_EXTENSIONS, TSLanguageDriver, TSParser, analyzeCircularDependencies, analyzeGitDiffImpact, analyzeImpact, analyzeProjectMetrics, apply, buildApiContractsResult, buildCircularResult, buildLintResult, buildMetricsResult, buildPathfindingResult, buildSliceResult, buildUnusedResult, extractSFCBlocks, findDomainSeedNodes, formatGraphMarkdown, generateMermaidDiagram, inject, kebabToPascal, name, normalizeApiPath, parseGoSource, parseJavaSource, parsePythonSource, parseRustSource, presentLensCall, presentLensResult, resolveModulePath };
