...
*/
function buildApiContractsResult(graph, fileSources) {
	const clientCalls = [];
	const serverEndpoints = [];
	if (fileSources) for (const [filePath, content] of fileSources.entries()) {
		const lines = content.split(/\r?\n/);
		for (let i = 0; i < lines.length; i++) {
			const line = lines[i];
			const lineNum = i + 1;
			const clientMatch = line.match(/\b(?:axios\.(get|post|put|delete|patch)|request\.(get|post|put|delete|patch)|fetch)\s*\(\s*['"`]([^'"`]+)['"`]/i);
			if (clientMatch) {
				const method = (clientMatch[1] || clientMatch[2] || "GET").toUpperCase();
				const url = clientMatch[3];
				if (url.startsWith("/") || url.startsWith("http")) clientCalls.push({
					filePath,
					url,
					method,
					line: lineNum
			...
			const springMatch = line.match(/@(?:(Get|Post|Put|Delete|Patch)Mapping|RequestMapping)\s*\(\s*(?:value\s*=\s*)?["']([^"']+)["']/i);
			if (springMatch) {
			...
			const goMatch = line.match(/\b(?:r|router|e|app|group)\.(GET|POST|PUT|DELETE|PATCH)\s*\(\s*["']([^"']+)["']/i);
			if (goMatch) {
...
function apply(ctx, config = {}) {
	...
	ctx.tools.register(defineTool({
		...
		async execute(args, options) {
			...
			if (lensArgs.action === "api_contracts") return buildApiContractsResult(store);
