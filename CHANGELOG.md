...
- **Top 10 最高优先级系统提示词**：将 Lens 代码智能指导提升为 Session 第一优先级（Order 10），显式注入 8 大用户自然语言意图映射（重构影响、调用链、前后端接口、依赖关系、Git 变更等）；
- **Code Mode 零摩擦执行约束**：强制告知模型直接调用 `return await tools.lens(...)`，彻底消除大模型盲目编写 `fs/glob/require/import` 脚本导致的沙箱权限拒绝与反复无效重试。
